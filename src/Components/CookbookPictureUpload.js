...
function CookbookPictureUpload({ cookbookID, type }) {
  ...
  const handleUpload = async () => {
    try {
      ...
      const response = await axios.post(`https://noahboyd-capstone-deploy-40896fd67e31.herokuapp.com/api/upload/${cookbookID}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          'Upload-Type': "cookbook" // Ensure this is correctly set based on the upload context
        },
  ...
  return (
    <Container className='upload-profile-container'>
      ...
      <button onClick={handleUpload}>Upload Cookbook Photo</button>
      <span style={statusText == "Success" ? {color: 'green'} : {color: 'red'}}>{statusText}</span>
    </Container>
  );
}

export default CookbookPictureUpload;
